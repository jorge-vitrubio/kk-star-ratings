    ...
    <h2 class="nav-tab-wrapper">
        <?php foreach ($tabs as $tab => $tabMeta) { ?>
            <a class="nav-tab <?php echo ($tabMeta['is_active'] ?? false) ? 'nav-tab-active' : ''; ?>"
                style="position: relative; border-radius: 4px 4px 0 0; <?php echo ($tabMeta['is_addon'] ?? false) ? (($tabMeta['is_active'] ?? false) ? 'background-color: auto; border-color: auto;' : 'background-color: #f6efc7; border-color: #ead2ae;') : ''; ?>"
                href="<?php echo admin_url('admin.php?page='.esc_attr($_GET['page'] ?? '').'&tab='.urlencode(esc_attr($tab))); ?>">
